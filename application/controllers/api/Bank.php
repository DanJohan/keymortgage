...
class Bank extends MY_Controller {
	...
	public function __construct()
	{
	    parent::__construct();
		 ...
		 $this->load->model('InsertUserBankModel');
	}
	...
	if(!empty($banks_list))
	{
		foreach ($banks_list as $banks) {
				...
				$bank_data[] = $results;
				$response = array('status'=>true,'message'=>'Record found successfully', 'bank_details'=>$bank_data);
...
public function addUserBank(){
	...
	public function addUsrBank(){
		$this->form_validation->set_rules('user_id', 'User id', 'trim|required');
		$this->form_validation->set_rules('bank_id', 'Bank id', 'trim|required');
	//	$this->form_validation->set_rules('bank_name', 'Bank name', 'trim|required');
		$this->form_validation->set_rules('user_bank_email', 'Bank user email', 'trim|required');
		...
		if ($this->form_validation->run() == true){
			$insert_data = array(
				...
				'bank_id' => $this->input->post('bank_id'),
				//'bank_name' => $this->input->post('bank_name'),
				'user_bank_email' => $this->input->post('user_bank_email'),
	...
	//user bank detail delete method
