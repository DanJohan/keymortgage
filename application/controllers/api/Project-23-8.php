...
/*		$page = $this->input->post('page');
	...
	if(!empty($project_list))
	{
		foreach ($project_list as $project) {
				...
				$results['descp'] = $project['description'];
				if(empty($project['project_image']))
				{
					$results['project_image'] = " ";
				}
				else
				{
				...
				$response = array('status'=>true,'message'=>'Record found successfully', 'project_details'=>$project_details);

                                      /*$response['project_details'][] = $results;

				if(!empty($project_list)){
			$response = array('status'=>true,'message'=>'Records get successfully','Projects'=>$project_list);
		}else{
